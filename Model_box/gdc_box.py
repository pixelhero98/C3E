...
from torch_geometric.nn import GCNConv, GATConv, GCN2Conv, GATv2Conv, SAGEConv
...
def train(model, data, p, optimizer, w):

    model.train()
    optimizer.zero_grad()
    out = model(data.x, data.edge_index, p, w)
...
@torch.no_grad()
def val(model, data, p, w):

    model.eval()
    pred = model(data.x, data.edge_index, p, w).argmax(dim=1)
