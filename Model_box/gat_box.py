...
class AAGNN(nn.Module):
    ...
    def __init__(self, num_mmp_layer, num_postpro_layer, num_skip_layer, head):
        ...
        self.mmp_layer = nn.ModuleList([GATConv(num_mmp_layer[2 * i], num_mmp_layer[2 * i + 1], heads=head) for i in range(len(num_mmp_layer) // 2)])
    ...
    def forward(self, x, a, p):
        ...
        for i in range(len(self.num_mmp_layer) // 2):
